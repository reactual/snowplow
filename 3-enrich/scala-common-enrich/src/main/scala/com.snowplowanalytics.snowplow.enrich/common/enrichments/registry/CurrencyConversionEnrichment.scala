...
object CurrencyConversionEnrichmentConfig extends ParseableEnrichment {
  ...
  def parse(
    ...
    isParseable(c, schemaKey).leftMap(e => NonEmptyList.one(e.toProcessingMessage)).flatMap { _ =>
      (
        ...
        CirceUtils
          ...
          .flatMap {
            ...
            case s => s"accountType [$s] is not one of DEVELOPER, ENTERPRISE, and UNLIMITED".asLeft
          }.toValidatedNel,
      ...
      }.toEither.leftMap(_.map(_.toProcessingMessage))
    }.toValidated
}
...
final case class CurrencyConversionEnrichment(
  ...
  private def performConversion(
    ...
    (initialCurrency, value) match {
      case (Some(ic), Some(v)) =>
        fx.convert(v, ic).to(baseCurrency).at(tstamp).bimap(
          l => {
