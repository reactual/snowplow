...
object Output {
  ...
  case object ExactlyOne extends ExpectedRowsMode {
    def collect(resultSet: List[JsonObject]): EitherThrowable[List[JsonObject]] =
      resultSet match {
        ...
        case other => InvalidDbResponse(s"SQL Query Enrichment: exactly one row was expected")
          .asLeft
      }
  }
  ...
  case object AtMostOne extends ExpectedRowsMode {
    def collect(resultSet: List[JsonObject]): EitherThrowable[List[JsonObject]] =
      resultSet match {
        ...
        case other => InvalidDbResponse(s"SQL Query Enrichment: at most one row was expected")
          .asLeft
      }
  }
  ...
  case object AtLeastOne extends ExpectedRowsMode {
    def collect(resultSet: List[JsonObject]): EitherThrowable[List[JsonObject]] =
      resultSet match {
        case Nil =>
          InvalidDbResponse(s"SQL Query Enrichment: at least one row was expected. 0 given instead")
            .asLeft
