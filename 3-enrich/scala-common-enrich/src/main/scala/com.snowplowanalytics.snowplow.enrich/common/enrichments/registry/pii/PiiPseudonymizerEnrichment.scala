...
object PiiPseudonymizerEnrichment extends ParseableEnrichment {
  ...
  def parse(
    ...
    for {
      ...
      piiFields <- CirceUtils
        .extract[List[Json]](conf, "parameters", "pii").toEither
      piiStrategy <- CirceUtils.extract[PiiStrategyPseudonymize](config, "parameters", "strategy")
        .toEither
  ...
  private def extractFields(piiFields: List[Json]): Either[String, List[PiiField]] =
    piiFields.map { json =>
      CirceUtils.extract[String](json, "pojo", "field").toEither
  ...
  private def extractPiiJsonField(jsonField: Json): Either[String, PiiJson] = {
    val schemaCriterion = CirceUtils.extract[String](jsonField, "schemaCriterion").toEither
    ...
    val mutator = CirceUtils.extract[String](jsonField, "field").toEither
      .flatMap(getJsonMutator)
      .toValidatedNel
    (mutator, schemaCriterion, jsonPath).mapN(PiiJson.apply)
