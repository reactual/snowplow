...
import utils.{ConversionUtils, CirceUtils}
...
object JavascriptScriptEnrichmentConfig extends ParseableEnrichment {
  ...
  def parse(
    c: Json,
    schemaKey: SchemaKey
  ): ValidatedNel[ProcessingMessage, JavascriptScriptEnrichment] = (for {
    ...
    .leftMap(_.toProcessingMessage).toEitherNel.toValidated
}
  ...
  private[registry] def compile(script: String): Either[String, Script] =
    Option(script) match {
      case Some(s) =>
        ...
        Either.catchNonFatal(cx.compileString(invoke, "user-defined-script", 0, null))
